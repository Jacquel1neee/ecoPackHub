...
<div class="card card-custom">
    <div class="card-body">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <p class="text-muted small mb-3">All existing variants will be replaced with the ones below.</p>

            <div id="variants-container">
                @php $existingVariants = $product->variants ?? collect(); @endphp
                    ...
                    @foreach($existingVariants as $index => $variant)
                        <div class="variant-item row g-3 mb-3 p-3" style="background: #f8f9fa; border-radius: 12px; border: 1px solid #e0e0e0;">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="variants[{{ $index }}][size]" class="form-control" placeholder="e.g., 600ml, 9inch" value="{{ $variant->size ?? 'Standard' }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label small">Packing Quantity *</label>
                                <input type="text" name="variants[{{ $index }}][packing_quantity]" class="form-control" placeholder="e.g., 400 pcs/ctn" value="{{ $variant->packing_quantity }}" required>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label small">Price (RM) *</label>
                                <input type="number" step="0.01" name="variants[{{ $index }}][price]" class="form-control" placeholder="45.00" value="{{ $variant->price }}" required>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label small">Stock *</label>
                                <input type="number" name="variants[{{ $index }}][stock]" class="form-control" placeholder="100" value="{{ $variant->stock }}" required>
                            </div>
                            <div class="col-md-2 d-flex align-items-end">
                                <button type="button" class="btn btn-danger btn-sm w-100 remove-variant" {{ $loop->first && $loop->last ? 'style=display:none;' : '' }}>
...
<script>
    let variantIndex = parseInt('{{ $product->variants->count() > 0 ? $product->variants->count() : 1 }}');
