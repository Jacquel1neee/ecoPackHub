                ...
                <tbody>
                    @forelse($orders as $order)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center align-items-center gap-1" style="flex-wrap: nowrap;">
                                    <!-- Update Status Form -->
                                    <form action="{{ route('admin.orders.update-status', $order) }}" method="POST" class="d-flex align-items-center gap-1" style="flex-wrap: nowrap;">
                                        @csrf
                                        @method('PATCH')
                                        <select name="status" class="form-select form-select-sm" style="width: 100px; font-size: 11px; padding: 2px 6px; border-radius: 6px;">
                                            <option value="pending" {{ $order->status == 'pending' ? 'selected' : '' }}>Pending</option>
                                            <option value="processing" {{ $order->status == 'processing' ? 'selected' : '' }}>Processing</option>
                                            <option value="completed" {{ $order->status == 'completed' ? 'selected' : '' }}>Completed</option>
                                            <option value="cancelled" {{ $order->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                        </select>
                                        <button type="submit" class="btn btn-sm btn-green" style="padding: 2px 8px; font-size: 12px;">
                                            <i class="fas fa-save"></i>
                                        </button>
                                    </form>
                                    <a href="{{ route('orders.show', $order) }}" class="btn btn-sm btn-outline-info" target="_blank" style="padding: 2px 8px; font-size: 12px;">
